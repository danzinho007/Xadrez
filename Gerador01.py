...
# Gerando os 100 mapas jogáveis
mapas = []
for i in range(100):
    # Gerando um novo mapa vazio
    novo_mapa = []
    for j in range(NUM_LINHAS):
        nova_linha = ""
        for k in range(NUM_COLUNAS):
            # Escolhendo um caractere aleatório da lista de caracteres permitidos
            novo_caractere = random.choice(CARACTERES_VALIDOS)
            # Garantindo que cada caixa esteja em um local correto
            if novo_caractere == "$":
                novo_caractere = "."
            nova_linha += novo_caractere
        novo_mapa.append(nova_linha)
    # Colocando o jogador em um local vazio
    while True:
        posicao_jogador = (random.randint(0, NUM_LINHAS-1), random.randint(0, NUM_COLUNAS-1))
    ...
    for j in range(3):
        while True:
            posicao_caixa = (random.randint(0, NUM_LINHAS-1), random.randint(0, NUM_COLUNAS-1))
